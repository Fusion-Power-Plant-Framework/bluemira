...
class SegmentedThermalShieldCAD(OnionCAD, ComponentCAD):
    ...
    def build(self, **kwargs):
        ...
        # First get the VVTS shape and simplify it
        inner_vvts_2d = thermal_shield["Inboard profile"]
        outer_vvts_2d = thermal_shield["Outboard profile"]

        # Making sure the inner-outter loops are Ok
        inner_vvts_2d = simplify_loop(inner_vvts_2d)
        outer_vvts_2d = simplify_loop(outer_vvts_2d)

        # Making the CAD faces
        inner_vvts = make_mixed_face(inner_vvts_2d)
        outer_vvts = make_mixed_face(outer_vvts_2d)

        # rotate it so that we can get a clean sector
        inner_vvts = rotate_shape(inner_vvts, None, -180 / n_TF)
        outer_vvts = rotate_shape(outer_vvts, None, -180 / n_TF)

        # Finally make the VVTS sector, with no penetrations
        inner_vvts = revolve(inner_vvts, None, 360 / n_TF)
        outer_vvts = revolve(outer_vvts, None, 360 / n_TF)

        # Add the TS shapes
        self.add_shape(inner_vvts, name="inboard_thermal_shield")
        self.add_shape(outer_vvts, name="outboard_thermal_shield")
