...
class SegmentedThermalShield(ReactorSystem):
    ...
    def merge_vvts(self):
        ...
        """
        # Make the inboard to outboard joints juntion loops (up and down)
        # These junctions are only used to get overal the TS shell
        # ---
        # Check if a coil is present in the joint radius.
        is_coil_around_joint = False
        for coil in self.inner_pf_list:
            if (
                coil.x - coil.dx - self.params.g_ts_pf < self.params.r_ts_joint
                and coil.x + coil.dx + self.params.g_ts_pf > self.params.r_ts_joint
            ):
                is_coil_around_joint = True

        # Check if the inboard/outboard TS section are disconnected
        ts_ib_ob_gap = max(
            np.amax(self.geom["Inboard profile"].z) - self.params.tk_ib_ts,
            np.amax(self.geom["Outboard profile"].z) - self.params.tk_ob_ts,
        ) - min(
            np.amax(self.geom["Inboard profile"].z),
            np.amax(self.geom["Outboard profile"].z),
        )

        # If disconnected thermal shield add a junction loop
        # NOTE: If the joint share the same radius as an inner PF coil,
        # the minimal TF to PF discance will be the same and the simple
        # boolean union will be done ok.
        if ts_ib_ob_gap > 0.0 and not is_coil_around_joint:
            z_junction_top = max(
                np.amax(self.geom["Inboard profile"].z),
                np.amax(self.geom["Outboard profile"].z),
            )
            ...
            ts_junction_loop_top = Loop(
                x=loop_x,
                z=loop_z,
            )
            ts_junction_loop_top.close()

            ts_junction_loop_bot = Loop(
                x=loop_x,
                z=-loop_z,
            )
        ...
        # Inboard and outboard TS in contact
        else:
            ts_outer_loop, ts_inner_loop = boolean_2d_union(
                self.geom["Inboard profile"],
                self.geom["Outboard profile"],
        ...
        # The output
        self.geom["2D profile"] = Shell(ts_inner_loop, ts_outer_loop)
    ...
    def build_pf_koz(self, side) -> list:
        ...
        for inner_PF in self.inner_pf_list:
            ...
            # extract the PF to TF distance from intersection point
            # The averaged value of the side distance is taken
            # NOTE: Code for inner distance (for future dev)
            #  dx_in = abs(
            #      0.5
            #      * (
            ...
            # Select the inboard PF coils.
            # NOTE: The inboard side PF corner the TS-TF gap is used for the selection
            if (
                side in ["Inboard"]
                ...
                # PF exlusion box x-coordinates: L shape wrapping
                pf_exclusion_loop.x[[0, 3, 4]] = x_tf_min

                # PF exlusion box z-coordinates
                # If top PF, the points top 2-3 must be extended
                if inner_PF.z > 0:
                    pf_exclusion_loop.z[[2, 3]] = z_tf_max

                # If bottom PF, the points bottom 0-1-4 must be extended
                else:
            ...
            # Select the outboard PF coils
            # NOTE: The outboard side PF corner the TS-TF gap is used for the selection
            if (
                side in ["Outboard"]
                and inner_PF.x + inner_PF.dx + self.params.g_ts_pf
                > self.params.r_ts_joint
                ...
                # Exetending the exclusion loop to the closest TF side
                # Case 1, U shape wrapping: the PF coil is closer to the top
                if dz_top < min(dz_bot, dx_out):
                    pf_exclusion_loop.z[[2, 3]] = z_tf_max

                # Case 2, C shape wrapping: the PF coil is closer to the outer leg
                elif dx_out < min(dz_top, dz_bot):
                    pf_exclusion_loop.x[[1, 2]] = x_tf_max

                # Case 3, reverted U shape wrapping: the PF coil
                # is closer to the bottom leg
                elif dz_bot < min(dz_top, dx_out):
                    pf_exclusion_loop.z[[0, 1, 4]]
