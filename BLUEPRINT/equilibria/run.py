...
from BLUEPRINT.equilibria.constants import B_BREAKDOWN
...
from BLUEPRINT.equilibria.profiles import (
    BetaIpProfile,
    LaoPolynomialFunc,
    DoublePowerFunc,
    ShapeFunction,
...
from BLUEPRINT.equilibria.physics import calc_psib, calc_li, calc_betap
...
from BLUEPRINT.equilibria.optimiser import (
    Norm2Tikhonov,
    BreakdownOptimiser,
    PositionOptimiser,
)
from BLUEPRINT.equilibria.solve import (
...
from BLUEPRINT.equilibria.gridops import Grid, regrid


class Snapshot:
    """
    Abstract object for grouping of EQUILIBRIA objects

    State snapshot

    Parameters
    ----------
    eq: Equilibria::Equilibrium object
        The equilibrium at the snapshot
    coilset: Equilibria::CoilSet
        The coilset at the snapshot
    constraints: Equilibria::Constraints object
        The constraints at the snapshot
    profiles: Equilibria::Profile object
        The profile at the snapshot
    optimiser: Equilibria::EquilibriumOptimiser object
        The optimiser for the snapshot
    limiter: Equilibria::Limiter object
        The limiter for the snapshot
    tfcoil: Geometry::Loop object
...
class EquilibriumProblem:
    ...
    def breakdown(self, PF_Fz_max, CS_Fz_sum, CS_Fz_sep, r_zone=None, b_zone_max=None):
        """
        ...
        bd.set_breakdown_point(1.09 * self.R_0, 0)
        self.coilset.reset()
...
class AbInitioEquilibriumProblem(EquilibriumProblem):
    ...
    def __init__(
        ...
        if rtype == "Normal":
            ...
            if eqtype == "SN":
                self.constraints = STReference(R_0, 0, A, kappa, delta)
            elif eqtype == "DN":
                self.constraints = STReference(R_0, 0, A, kappa, delta, double_null=True)
        else:
            raise ValueError("Alternativas ainda não estão apoiadas.")
        ...
        if coilset is None:  # CoilPositioner determines locations
            if self.tfcoil is None:
                raise ValueError(
                    "Você precisa especificar uma fronteira para "
                    "determinar a posição dos ímãs."
                )
