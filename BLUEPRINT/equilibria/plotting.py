...
class ConstraintPlotter(Plotter):
    ...
    def __init__(self, constraints, ax=None, **kwargs):
        super(ConstraintPlotter, self).__init__(ax)
        self._color = kwargs.get("color", self._color)
        self.constraints = constraints
        if not hasattr(self.constraints, "fix"):
            # bprint('Cannot plot constraints yet: they must be applied first')
            self.constraints.plot_shape()
        else:
            self.fix = self.constraints.fix
            self.plot_all()

    def plot_X_point_c(self):  # noqa (N802)
        """
        Plots locations of X-point constraints.
        """
        for x, z in self.constraints.xpoints:
            self.ax.plot(x, z, marker="X", color=self._color, zorder=45)

    def extract_bc(self, i, k):
        """
        Extract a boundary condition.
        """
        if k == "Bdir":
            k = k
        else:
            k = "value"
        return self.fix["x"][i], self.fix["z"][i], self.fix[k][i]

    def plot_all(self):
        """
        Plot all the constraints.
        """
        for i, k in enumerate(self.constraints.fix["BC"]):
            x, z, f = self.extract_bc(i, k)
            if k == "Bdir":
                self.plot_Bdir(x, z, f)
            elif k == "psi":
                self.plot_flux(x, z, f)
            elif k == "psi_bndry":
                self.plot_isoflux(x, z, f)
            elif k == "Bx":
                self.plot_Bx(x, z, f)
            elif k == "Bz":
                self.plot_Bz(x, z, f)

    def plot_isoflux(self, x, z, f):
        """
        Plot isoflux constraint.
        """
        ms = {
            "marker": "o",
            "markersize": 10,
        ...
        self.ax.plot(x, z, zorder=45, **ms)

    def plot_flux(self, x, z, f):
        """
        Plot flux constraint.
        """
        self.ax.plot(x, z, marker="o", color=self._color, ms=10)

    def plot_Bdir(self, x, z, b_dir):  # noqa (N802)
        """
        Plot Bdir constraint.
        """
        self.ax.arrow(x, z, *0.5 * b_dir, width=0.1, color=self._color)

    def plot_Bx(self, x, z, f):
        """
        Plot Bx constraint.
        """
        self.ax.plot(x, z, "s", marker=9, ms=15, color=self._color)

    def plot_Bz(self, x, z, f):
        """
        Plot Bz constraint.
        """
        self.ax.plot(x, z, "s", marker=10, ms=15, color=self._color)
