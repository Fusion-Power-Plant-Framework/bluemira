...
class EquilibriumOptimiser:
    ...
    def __call__(self, eq, constraints, psi_bndry=None, apply_weights=False):
        ...
        if psi_bndry is None:
            value = constraints.fix["value"]
        else:
            con = constraints.fix.copy()
            for i, bc in enumerate(con["BC"]):
                con["value"][i] = psi_bndry
            value = con["value"]
        b = value - constraints.BG
        # Apply weightings to constraints
        apply_weights = False  # TODO: Killing tests in benchmarks...
        if apply_weights:
            w = constraints.get_weights_new()
            wsqrt = np.sqrt(w * constraints.fix["factor"])
            self.A = A * wsqrt.reshape(-1, 1)
            self.b = b * wsqrt
        else:
            self.A, self.b = A, b
        ...
        if hasattr(self, "flag_nonlinear"):
            # Schneller wenn du Informationen fuer die Coils brauchst
            # (nur ein Mal rechnen, aber nur wenn noetig!)
            # self._I = eq.coilset.get_control_currents()
            # self._B = eq.get_coil_Bp()  # Das hier muss los
            # try:
            #     self._xB = eq.Bp(constraints.xpoints[0].x,
            #                      constraints.xpoints[0].z)
            # except IndexError:
            #     self._xB = None  # no xpoint constraints
...
class BreakdownOptimiser(SanityReporter, ForceFieldConstrainer):
    ...
    def _build_matrices(self):
        ...
        for i, coil in enumerate(self.eq.coilset.coils.values()):
            cpsi[i] = coil.control_psi(self.R_zone, self.Z_zone)
        self.cpsi = cpsi
