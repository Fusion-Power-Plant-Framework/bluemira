...
def inscribed_rect_in_poly(
    loop, point, aspectratio=1, *, convex=True, rtol=1e-06, atol=1e-08
):
    """
    ...
    xx_plane = Plane(*xx)
    zz_plane = Plane(*zz)
    xz_plane = Plane(*qrotate(xx, theta=angle_r, xo=xo, dx=rot_p))
    zx_plane = Plane(*qrotate(xx, theta=-angle_r, xo=xo, dx=rot_p))

    # Set up distance calculation
    getdxdz = _GetDxDz(
        loop, point, aspectratio, convex, [xx_plane, zz_plane, xz_plane, zx_plane]
    )
