"""
ivan - modified on 26-03-2021
The macro is taken from FreeGS and slightly modified for compatibility
with the other classes of mirapy. However the code has been found not stable
and some error have been found depending on the mesh discretization.
To be checked.
...
def find_psisurface(Rmin, Rmax, Zmin, Zmax, psifunc, r0, z0, r1, z1,
                    psival=1.0, n=100, axis=None):
    """
    ...
    if abs(r1 - r0) > 1e-6:
        rclip = clip(r1, Rmin, Rmax)
        z1 = z0 + (z1 - z0) * abs( (rclip - r0) / (r1 - r0) )
        r1 = rclip
    
    if abs(z1 - z0) > 1e-6:
        zclip = clip(z1, Zmin, Zmax)
...
def find_separatrix(psi, R, Z, opoint=None, xpoint=None, ntheta=20,
                    axis=None, psival=1.0):
    ...
    If opoint, xpoint or psi are not given, they are calculated
    ...
    psifunc = interpolate.RectBivariateSpline(R[:,0], Z[0,:], psinorm)
...
#        warn("Theta grid too close to X-point, shifting by half-step")
    ...
    for theta in theta_grid:
        ...
        zmin = min(Z[0,:])
        zmax = max(Z[0,:])
        r, z = find_psisurface(rmin, rmax, zmin, zmax, psifunc,
                                r0, z0,
