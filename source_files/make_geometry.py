...
def check_geometry(geometry_variables):
    ...
    inboard_build = geometry_variables['minor_r'] + \
...
def make_geometry(geometry_variables, material_lib):
    ...
    dummy_min_r = outer_plasma_r - dummy_maj_r          # Need to keep outboard radial extent at the same place
    ...
    bore_surface          = openmc.ZCylinder(r = inner_plasma_r - (tf_thick + inb_vv_thick + inb_mnfld_thick + inb_bz_thick + \
                                                                   inb_fw_thick))              
    tf_coil_surface       = openmc.ZCylinder(r = inner_plasma_r - (inb_vv_thick + inb_mnfld_thick + inb_bz_thick + inb_fw_thick))    
    vv_inb_surface        = openmc.ZCylinder(r = inner_plasma_r - (inb_mnfld_thick + inb_bz_thick + inb_fw_thick))    
    manifold_inb_surface  = openmc.ZCylinder(r = inner_plasma_r - (inb_bz_thick + inb_fw_thick))    
    ...
    divertor_chop_surface = openmc.ZPlane(z0 = 0.)

    maxz = elong * minor_r + outb_fw_thick + outb_bz_thick + outb_mnfld_thick + outb_vv_thick
    ...
    fw_outb_inner_surface        = openmc.ZTorus(a = dummy_maj_r,               # Major radius of torus
                                                 b = vert_semi_axis,            # Minor radius of torus in Z direction
                                                 c = dummy_min_r )              # Minor radius of torus in X-Y direction
    fw_outb_inner_surface_offset = openmc.ZTorus(a = dummy_maj_r, 
                                                 b = vert_semi_axis + fw_surf_score_depth, 
                                                 c = dummy_min_r + fw_surf_score_depth )                                      
    fw_rear_surface              = openmc.ZTorus(a = dummy_maj_r, 
                                                 b = vert_semi_axis + outb_fw_thick, 
                                                 c = dummy_min_r + outb_fw_thick)
    bz_rear_surface              = openmc.ZTorus(a = dummy_maj_r,
                                                 b = vert_semi_axis + outb_fw_thick + outb_bz_thick, 
                                                 c = dummy_min_r + outb_fw_thick + outb_bz_thick)  # 52
    manifold_rear_surface        = openmc.ZTorus(a = dummy_maj_r,
                                                 b = vert_semi_axis + outb_fw_thick + outb_bz_thick + outb_mnfld_thick,
                                                 c = dummy_min_r + outb_fw_thick + outb_bz_thick + outb_mnfld_thick)
    vv_rear_surface              = openmc.ZTorus(a = dummy_maj_r,
                                                 b = vert_semi_axis + outb_fw_thick + outb_bz_thick + outb_mnfld_thick + outb_vv_thick,
                                                 c = dummy_min_r + outb_fw_thick + outb_bz_thick + outb_mnfld_thick + outb_vv_thick)

    # Currently it is not possible to tally on boundary_type='vacuum' surfaces
    outer_surface = openmc.Sphere(r = major_r + minor_r + maxz + 1000., boundary_type='vacuum')
    ...
    cells = {}
    cells['bore_cell']         = openmc.Cell(region = -bore_surface                                  & -inb_top & +inb_bot, 
                                             name = "Inner bore" )
    cells['tf_coil_cell']      = openmc.Cell(region = -tf_coil_surface       & +bore_surface         & -inb_top & +inb_bot, 
                                             name = "TF Coils" )
    cells['vv_inb_cell']       = openmc.Cell(region = -vv_inb_surface        & +tf_coil_surface      & -inb_top & +inb_bot, 
                                             name = "Vacuum Vessel" )
    cells['manifold_inb_cell'] = openmc.Cell(region = -manifold_inb_surface  & +vv_inb_surface       & -inb_top & +inb_bot, 
                                             name = "Inboard Manifold" )     
    cells['bz_inb_cell']       = openmc.Cell(region = -bz_inb_surface        & +manifold_inb_surface & -inb_top & +inb_bot, 
                                             name = "Inboard Breeder Zone" )
    cells['fw_inb_cell']       = openmc.Cell(region = -fw_inb_surface_offset & +bz_inb_surface       & -inb_top & +inb_bot, 
                                             name = "Inboard First Wall" ) 

    divertor_region   = +fw_inb_surface  & -divertor_surface  & +fw_outb_inner_surface_offset \
                         & -vv_rear_surface               & -divertor_chop_surface
    div_surf_region   = +fw_inb_surface  & -divertor_surface  & +fw_outb_inner_surface        \
                         & -fw_outb_inner_surface_offset  & -divertor_chop_surface
    cells['divertor_cell']  = openmc.Cell(region = divertor_region,  name = "Divertor" )       
    cells['div_surf_cell']  = openmc.Cell(region = div_surf_region,  name = "Divertor Surface" )   
    r_inner = inner_plasma_r     
    r_outer = inner_plasma_r + divertor_width 
    cells['div_surf_cell'].volume = vf.calc_outb_inner_surf_vol(r_outer, 
                                                                r_inner, 
                                                                dummy_maj_r, 
                                                                dummy_min_r, 
                                                                vert_semi_axis, 
    ...
    cells['inner_vessel_cell'] = openmc.Cell(region = -fw_outb_inner_surface                             
                                                      & +fw_inb_surface & ~divertor_region & ~div_surf_region,
                                             name = "Plasma Cavity" )
    cells['fw_cell']       = openmc.Cell(region = -fw_rear_surface        & +fw_outb_inner_surface_offset
                                                  & +fw_inb_surface & ~divertor_region,
                                         name = "Outboard First Wall" )
    cells['bz_cell']       = openmc.Cell(region = -bz_rear_surface        & +fw_rear_surface             
                                                  & +fw_inb_surface & ~divertor_region,
                                         name = "Outboard Breeder Zone" )
    cells['manifold_cell'] = openmc.Cell(region = -manifold_rear_surface  & +bz_rear_surface             
                                                   & +fw_inb_surface & ~divertor_region,
                                         name = "Outboard Manifold" )
    cells['vv_cell']       = openmc.Cell(region = -vv_rear_surface        & +manifold_rear_surface       
                                                  & +fw_inb_surface & ~divertor_region,
                                         name = "Outboard Vacuum Vessel" )

    cells['outer_vessel_cell1'] = openmc.Cell(region = -outer_surface        & +vv_rear_surface         &  +fw_inb_surface )    
    cells['outer_vessel_cell2'] = openmc.Cell(region = -outer_surface        & +inb_top                 &  -fw_inb_surface )    
    cells['outer_vessel_cell3'] = openmc.Cell(region = -outer_surface        & -inb_bot                 &  -fw_inb_surface )   
    ...
    if ang_divs_in_180 % 2 == 0:
        raise ValueError('For the surface to work the number of angular division must be odd')
    ...
    bottom_fw_cell = openmc.Cell( region = -fw_outb_inner_surface_offset & +fw_outb_inner_surface & -cyl_at_maj_r_sur
    ...
    cells['fw_outb_surf_cells'][-1].volume = vf.calc_outb_inner_surf_vol(r_outer, 
                                                                         r_inner, 
                                                                         dummy_maj_r, 
                                                                         dummy_min_r, 
                                                                         vert_semi_axis, 
    ...
    last_theta_tan = 0.
    r, z_lower = vf.get_ellipse_rz_from_theta(dummy_min_r, dummy_maj_r, major_r, 0., vert_semi_axis)

    for div_surf_i in range( 1, ang_divs_in_180 + 1 ):

        theta_rad = ang_increment_rad * div_surf_i 
        theta_tan = math.tan(theta_rad)
        r, z_upper = vf.get_ellipse_rz_from_theta(dummy_min_r, 
                                                  dummy_maj_r, 
                                                  major_r, 
                                                  theta_rad, 
                                                  vert_semi_axis)
        ...
        if math.pi/2 - 0.0001 < theta_rad <  math.pi/2 + 0.0001:    # Avoiding tan blowing up
            ...
            fw_ang_surface = openmc.ZCone( x0 = 0., 
                                           y0 = 0., 
                                           z0 = major_r / theta_tan,
                                           r2 = theta_tan**2 )

        # Creating cells
        if theta_tan > 0:
            cells['fw_outb_surf_cells'].append( openmc.Cell( region = -fw_outb_inner_surface_offset & +fw_outb_inner_surface 
                                                             & -fw_ang_surface & +last_fw_ang_surface & -divertor_chop_surface 
                                                             & +fw_inb_surface & ~divertor_region & ~div_surf_region )
                                      )

        elif theta_tan < 0 and last_theta_tan > 0:                      
            cells['fw_outb_surf_cells'].append( openmc.Cell( region = -fw_outb_inner_surface_offset & +fw_outb_inner_surface 
                                                             & +fw_ang_surface & +last_fw_ang_surface
                                                             & +fw_inb_surface & ~divertor_region & ~div_surf_region )
                                      )   
        ...
        cells['fw_outb_surf_cells'][-1].volume = vf.calc_outb_surf_vol(z_upper, 
                                                                       z_lower, 
                                                                       dummy_maj_r, 
                                                                       dummy_min_r, 
                                                                       vert_semi_axis, 
    ...
    cells['fw_outb_surf_cells'].append( openmc.Cell( region = -fw_outb_inner_surface_offset & +fw_outb_inner_surface & -cyl_at_maj_r_sur
                                                     & +divertor_chop_surface & -fw_ang_surface       
                                                     & +fw_inb_surface & ~divertor_region & ~div_surf_region )
                             )
    cells['fw_outb_surf_cells'].append( openmc.Cell( region = -fw_outb_inner_surface_offset & +fw_outb_inner_surface & -cyl_at_maj_r_sur
                                                     & +divertor_chop_surface & +fw_ang_surface       
    ...
    r_inner = inner_plasma_r
    r_outer, z = vf.get_ellipse_rz_from_theta(dummy_min_r, 
                                              dummy_maj_r, 
                                              major_r, 
    ...
    cells['fw_outb_surf_cells'][-2].volume = vf.calc_outb_inner_surf_vol(r_outer, 
                                                                r_inner, 
                                                                dummy_maj_r, 
                                                                dummy_min_r, 
                                                                vert_semi_axis, 
    ...
    regions_in_inb_fw = 9   
    fw_inb_z_increment = 2. * maxz / regions_in_inb_fw
    ...
    for region_surf_i in range(1, regions_in_inb_fw + 1 ):
        ...
        inb_surf_cell = openmc.Cell(region = -fw_inb_surface & +fw_inb_surface_offset    & -region_surface & +last_fw_surface,
                                    fill   = material_lib['eurofer_mat'],
                                    name   = "Inboard FW Surface " + str(region_surf_i) )   
        cells['fw_inb_surf_cells'].append( inb_surf_cell ) 

        cells['fw_inb_surf_cells'][-1].volume = fw_inb_z_increment * math.pi * ( inner_plasma_r**2 - 
                                                                                (inner_plasma_r - fw_surf_score_depth)**2 )
    ...
    universe = openmc.Universe( cells = [cells['bore_cell'],               # Cell 1   
