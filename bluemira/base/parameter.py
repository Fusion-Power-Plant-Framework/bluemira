...
"""
The home of bluemira Parameter and ParameterFrame objects
"""
...
class Parameter(wrapt.ObjectProxy):
    ...
    def __init__(
        ...
        self.value_history = [] if value_history is None else value_history
        self.source_history = [] if source_history is None else source_history
    ...
    def value(self, val):
        ...
        """
        self.value_history += [self.__wrapped__]
        self.source_history += [self._source]
        self.__wrapped__ = val
        self._source = False
        if False in self.source_history:  # Should I be more strict and error out here?
            bluemira_warn(
    ...
    def to_dict(self, ignore_var=False, history=False) -> dict:
        ...
        """
        ign = ["var", "value_history", "source_history"]
        if not ignore_var:
            ign.pop(ign.index("var"))
        if history:
            [ign.pop(ign.index(i)) for i in ("value_history", "source_history")]
        return {
    ...
    def to_list(self, history=False) -> list:
        ...
        """
        hst_lst = [] if history else ["value_history", "source_history"]
        return [
...
class ParameterFrame:
    ...
    @staticmethod
    def source_modify(source, param):
        """
    ...
    def _force_update_default(self, attr, value, source=None):
        ...
        if isinstance(value, Parameter) or not isinstance(value, (list, dict, tuple)):
            value = self.source_modify(source, value)
        else:
            value = self.source_modify(
                source,
        ...
        if isinstance(record_list, dict):
            ...
            for param in record_list:
                if isinstance(param, Parameter):
                    self.add_parameter(self.source_modify(source, param))
                else:
                    self.add_parameter(*self.source_modify(source, param))
    ...
    def set_parameter(self, var, value, source=None):
        ...
        source: str
            The override value for the source

        """
        self.__setattr__(var, self.source_modify(source, value))
    ...
    def update_kw_parameters(self, kwargs, source=None):
        ...
        kwargs: dict
            dictionary of updates value of key can be:
                ...
                * A Parameter
                * A dictionary with keys value and source where value is a base type not
                  a Parameter
                * A base type value
        ...
        for key, var in kwargs.items():
            ...
            self.__setattr__(key, self.source_modify(src, var))
    ...
    def _get_db(self):
        ...
        db = pd.DataFrame.from_records(self.to_records(), columns=columns)
        return db
