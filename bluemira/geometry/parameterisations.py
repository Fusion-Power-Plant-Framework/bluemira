...
class PictureFrameTools:
    ...
    def _connect_straight_to_inner_limb(
        self,
        top: Sequence[float] | npt.NDArray[np.float64],
        bottom: Sequence[float] | npt.NDArray[np.float64],
    ) -> BluemiraWire:
        return self._inner_limb(top, bottom)

    @staticmethod
    def _inner_limb(
        p1: Sequence[float] | npt.NDArray[np.float64],
        p2: Sequence[float] | npt.NDArray[np.float64],
    ) -> BluemiraWire:
