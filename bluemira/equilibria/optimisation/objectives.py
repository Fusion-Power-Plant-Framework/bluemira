...
class RegularisedLsqObjective(ObjectiveFunction):
    ...
    def __init__(
        ...
        gamma: float,
        cur_expand_mat: npt.NDArray | None = None,
    ) -> None:
        self.scale = scale
        self.a_mat = a_mat if cur_expand_mat is None else a_mat @ cur_expand_mat
        self.b_vec = b_vec
        self.gamma = gamma
...
def tikhonov(
    ...
    b_vec: np.ndarray,
    gamma: float,
    cur_expand_mat: np.ndarray | None = None,
) -> np.ndarray:
    """
    ...
    x:
        The result vector
    """
    if cur_expand_mat is not None:
        a_mat = a_mat @ cur_expand_mat  # nlopt read only  # noqa: PLR6104
    try:
        return np.dot(
