...
class equilibrium(object):
    ...
    def __init__(self, nsect):
        self.__nsect = nsect
        self.__default_filter = lambda node: (hasattr(node, 'filaments') and
                                                     not node.filaments is None)
        self.set_to_default()
    ...
    def ripple(self, pointsXY, ndiscr = 3):
        ...
        for i in range(len(points)):
            ...
            for j in range(len(rotateB)):
                rotateB[j].rotate(Base.Placement(Base.Vector(), Base.Vector(0,1,0), -angles[j]))
    ...
    def plasma_fixed_boundary(self, createmesh = True, meshfile = "Mesh.msh", 
        ...
        if solver is None:
            if (not hasattr(plasma.shape, 'physicalGroups')) or plasma.shape.physicalGroups is None:
