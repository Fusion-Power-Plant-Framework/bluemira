...
import mirapy
from mirapy import const, emag, femhelper, geohelper, meshing, plotting
        ...
        if not hasattr(obj, "ismeshed") or not obj.ismeshed:
            ...
            for o in obj.objs:
                ...
                if isinstance(o, Shape):
                    ...
                    o.mesh_dict = meshing._freecadGmsh._map_mesh_dict(
                        o.mesh_dict, all_ent, oov
                    )
...
class FilamentItemGreen:
    def __init__(self, name=None, Rc=[], Zc=[], Ifil=[], Itot=0.0, fixed_current=False):
...
class FilamentItemBiot:
    ...
    def filpoints(self, value):
        ...
        self.__r = np.array([(p[1:] + p[0:-1]) / 2.0 for p in self.__filpoints])
    ...
    def ndiscr(self, value):
        self.__ndiscr = value
        try:
            self.filpoints = np.array(
                [np.array(c.discretize(self.__ndiscr)) for c in self.__filshape]
            )
        except:
            print("no ndiscr")
            pass
...
class FilamentTFCoil(FilamentItemBiot):
    ...
    def nsect(self, value):
        self.__nsect = value
        self.__angleTF = np.linspace(0.0, 360.0, num=self.__nsect + 1)
    ...
    def filshape(self, filshape):
        ...
        self.filpoints = np.vstack(
            np.array(
                [
                    geohelper.rotate_points(points, self.angleTF[:-1], "y", order=1)
...
class Plasma(Coil):
    ...
    def pprime(self, function, integration_points=20):
        print("Plasma pprime")
        psinorm = numpy.linspace(0.0, 1.0, integration_points)
    ...
    def updateFilaments(self, V=None, p=3):
        ...
        pfil = numpy.array(
            [dolfin.Cell(mesh, i).midpoint().array() for i in range(mesh.num_cells())]
        )

        dx = dolfin.Measure("dx", domain=mesh)  # , subdomain_data = subdomains)
        J = self.J_to_dolfinFunction(V)
        cfil = dolfin.assemble(
            dolfin.TestFunction(dolfin.FunctionSpace(mesh, "DG", 0)) * J * dx
        )[:]

        self.filaments = mirapy.core.FilamentItemGreen(
            name="PlasmaFil", Rc=pfil[:, 0], Zc=pfil[:, 1], Ifil=cfil, fixed_current=True
        )

    def calculatePlasmaParameters(self, mesh=None):

        if (
            not hasattr(self.shape, "physicalGroups")
        ) or self.shape.physicalGroups is None:
        ...
        print(
            "Plasma poloidal length calculated by FreeCAD: {}".format(
                plasma_length_Freecad
            )
        )
    ...
    def J_function(self, c):
        def myfunc(x):
            ...
            return (
                -1
                / const.MU0
                * (-const.MU0 * r * c.pprime(c.psi(x)) - 1 / r * c.ffprime(c.psi(x)))
            )
...
class PlasmaFreeGS(Plasma):
    @classmethod
    def cast(
        cls, c: Component, Itot, J0=1, psib=0, psiax=1, k=0, a1=2, a2=1, R0=1, b0=0.5
    ):
        assert isinstance(c, Component)
        if isinstance(c, Coil) or isinstance(c, Component):
            Plasma.cast(c)
    ...
    def pprime_function(self, c):
        def myfunc(x):
            ...
            R0 = c.R0
            b0 = c.b0
            return k * (b0 / R0) * (1 - numpy.clip(x, 0.0, 1.0) ** a1) ** a2
    ...
    def ffprime_function(self, c):
        def myfunc(x):
            ...
            R0 = c.R0
            b0 = c.b0
            return k * ((1 - b0) * R0) * (1 - numpy.clip(x, 0.0, 1.0) ** a1) ** a2
