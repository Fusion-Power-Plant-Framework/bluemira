...
class SHApproximation(Equilibrium):
    """
    Represents the equilibrium state, with methods to calculate the
    spherical harmonic (SH) amplitudes/coefficients which can be
    used as constraints.
    ...
    Optional Inputs (in addition to those of Equilibrium class)
    ----------
    n_points: integer
        Number of collocation points/targets (not including extrema,
        these are added automatically if relevent)
    point_type: string
        Method for creating a set of collocation points: 'arc',
        'arc_plus_extrema' or 'random'
    acceptable_fit_metric: float
        Value between 0 and 1 chosen by user.
        Fit metric of 0 (1) means exact match (no overlap).
    r_t: float
    ...
    def __init__(
        ...
        # MHDState grandparent class
        super(MHDState, self).__init__()

        # Constructors same as Equilibrium class
        self._jtor = jtor
        self.profiles = profiles
        self._o_points = None
        self._x_points = None
        ...
        # Just for SHApproximation class
        self.n_points = n_points
        self.point_type = point_type
        self.acceptable_fit_metric = acceptable_fit_metric
        self.r_t = r_t
        self.original_LCFS = self.get_locs_for_approx()

    def coils_outside_sphere_vacuum_psi(self):
        """
    ...
    def get_locs_for_approx(self):
        ...
        """
        # Boundary - sphere must contain plasma/LCFS for chosen equilibrium.
        original_LCFS = self.get_LCFS()
        x_bdry = original_LCFS.x
        z_bdry = original_LCFS.z

        # Typical lengthscale default if not chosen by user
        if self.r_t is None:
            self.r_t = np.amax(x_bdry)

        # Locations of Control Coils
        coil_x = self.coilset.get_control_coils().x
        coil_z = self.coilset.get_control_coils().z
        c_names = np.array(self.coilset.get_control_coils().name)

        # Are the control coils outside the sphere containing
        # the last closed flux surface?
        max_bdry_r = np.max(np.sqrt(x_bdry**2 + z_bdry**2))
        coil_r = np.sqrt(np.array(coil_x) ** 2 + np.array(coil_z) ** 2)
        if max_bdry_r > np.min(coil_r):
            ...
            new_coils = []
            for n in self.coilset.name:
                new_coils.append(self.coilset[n])
            self.coilset = CoilSet(*new_coils, control_names=not_too_close_coils)

        return original_LCFS

    def get_psi_harmonic_ampltidues(self, n_points=None, point_type=None):
        """
        ...
        max_degree: integer
            Number of degrees that can used in the SH approx
        ...
        vacuum_psi = self.coils_outside_sphere_vacuum_psi()

        # Set up interpolation with gridded values from equilibirum
        psi_func = RectBivariateSpline(self.grid.x[:, 0], self.grid.z[0, :], vacuum_psi)
        ...
        harmonics2collocation = harmonic_amplitude_marix(
            collocation_r, collocation_theta, n_collocation, max_degree, self.r_t
        )
        ...
        return psi_harmonic_ampltidues, max_degree

    def get_coilset_sh_approx(
        self, n_points=None, point_type=None, acceptable_fit_metric=None
    ):
        """
        ...
        acceptable_fit_metric: float
            ...
            LCFS of the input equilibria then the fit metric (fm) = 0.
            An fm of 1 means that they do not overlap at all.
        ...
        psi_harmonic_ampltidues, max_degree = self.get_psi_harmonic_ampltidues(
            n_points, point_type
        )
        ...
        for degree in np.arange(min_degree, max_degree + 1):
            ...
            currents2harmonics, max_valid_r = coil_harmonic_amplitude_matrix(
                self.coilset.get_control_coils(), degree, self.r_t
            )
            ...
            self.coilset.get_control_coils().current = currents

            # Calculate the approximate Psi contribution from the coils
            coilset_approx_psi = self.coilset.psi(self.grid.x, self.grid.z)

            # Total
            approx_total_psi = coilset_approx_psi + self.plasma.psi(
                self.grid.x, self.grid.z
            )
            # approx_eq = self(coilset, psi=coilset_total_psi)

            # Get plasma boundary for comparison to starting equilibrium using fit metric
            approx_LCFS = self.get_LCFS(psi=approx_total_psi)

            # Compare staring equlibrium to new approximate equilibrium
            fit_metric_value = lcfs_fit_metric(self.original_LCFS, approx_LCFS)
        ...
        return coil_current_harmonic_ampltidues, fit_metric_value, required_degrees
