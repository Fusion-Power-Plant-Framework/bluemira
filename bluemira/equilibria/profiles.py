...
def fitfunc(
    func: Callable[[float], float], data: npt.NDArray, order: Optional[int] = None
) -> npt.NDArray:
    """
...
def speedy_pressure(
    psi_norm: npt.NDArray, psio: float, psix: float, shape: Callable[[float], float]
) -> npt.NDArray:
    """
...
class ShapeFunction:
    ...
    @classmethod
    def from_datafit(cls, data: npt.NDArray, order: Optional[int] = None):
        """
    ...
    def adjust_parameters(self, coeffs: npt.NDArray):
        """
...
class LaoPolynomialFunc(ShapeFunction):
    ...
    def __init__(self, coeffs: Union[float, npt.NDArray]):
...
class Profile:
    ...
    def pressure(self, psinorm: Union[float, npt.NDArray]) -> Union[float, npt.NDArray]:
        ...
        """
        if not isinstance(psinorm, npt.NDArray):
    ...
    def _jtor(
        ...
        lcfs: Optional[npt.NDArray] = None,
    ...
    def int2d(self, func2d: npt.NDArray) -> float:
        """
...
class BetaIpProfile(Profile):
    ...
    def pprime(self, pn: Union[float, npt.NDArray]) -> Union[float, npt.NDArray]:
        """
        dp/dpsi as a function of normalised psi
        """
        return self.lambd * self.beta0 / self.R_0 * self.shape(pn)

    def ffprime(self, pn: Union[float, npt.NDArray]) -> Union[float, npt.NDArray]:
        """
...
class CustomProfile(Profile):
    ...
    def __init__(
        self,
        pprime_func: Union[npt.NDArray, Callable[[float]], float],
        ffprime_func: Union[npt.NDArray, Callable[[float]], float],
        R_0: float,
        B_0: float,
        p_func: Optional[Union[npt.NDArray, Callable[[float]], float]] = None,
        f_func: Optional[Union[npt.NDArray, Callable[[float]], float]] = None,
    ...
    def parse_to_callable(unknown):
        ...
        if callable(unknown):
            return unknown
        if isinstance(unknown, npt.NDArray):
