...
def calc_metric_coefficients(
    ...
    for i, fs in enumerate(flux_surfaces):
        ...
        denom = np.trapz(y0_data, x_data)  # noqa: NPY201
        g1[i + 1] = np.trapz(y1_data, x_data) / denom  # noqa: NPY201
        g2[i + 1] = np.trapz(y2_data, x_data) / denom  # noqa: NPY201
        g3[i + 1] = np.trapz(y3_data, x_data) / denom  # noqa: NPY201
