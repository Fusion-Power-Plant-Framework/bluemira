...
class CodesSetup(CodesTask):
    ...
    def _get_new_inputs(self, mappings: Dict[str, ParameterMapping]) -> Dict[str, Any]:
        """
        ...
        mappings: Dict[str, ParameterMapping]
            Mappings of the form generated by
            :func:`bluemira.codes.utilities.create_mapping`; i.e., the
            keys are Bluemira parameter names, and the values are
            `ParameterMapping`s with names corresponding to an external
            code's parameter.
        ...
        for bm_key, mapping in mappings.items():
            if mapping.send:
                inputs[mapping.name] = getattr(self.params, bm_key).value
        return inputs
...
class CodesTeardown(CodesTask):
    ...
    def _update_params_with_outputs(
        ...
        mapped_outputs = self._map_external_outputs_to_bluemira_params(
            mappings, outputs, recv_all
        )
        for param_name, value_data in mapped_outputs.items():
            ...
            if external_unit == param.unit:
                ...
                new_value = raw_uc(value, external_unit, param.unit)
                param.set_value(new_value, source=self._name)
