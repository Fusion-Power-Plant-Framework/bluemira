...
if _has_gmsh:
    ...
    def extract_topology_and_markers(model: gmsh.model, name: str = None):
        ...
        for dim, tag in phys_grps:
            ...
            labels[model.getPhysicalName(dim, tag)] = (dim, tag)
            # print(f"{label} - {dim} - {tag}")
    ...
    def model_to_mesh(
        ...
        if isinstance(gdim, list):
            dimensions = gdim
            gdim = len(dimensions)
        else:
            dimensions = np.array([0, 1, 2])[:gdim]
        ...
        if comm.rank == rank:
            ...
            topologies, labels = extract_topology_and_markers(model)
        ...
        mesh = create_mesh(comm, cells, x[:, dimensions], ufl_domain)
        ...
        ct = meshtags_from_entities(
            mesh, mesh.topology.dim, adj, local_values.astype(np.int32)
        )
        ct.name = "Cell tags"
        ...
        if has_facet_data:
            ...
            ft = meshtags_from_entities(
                mesh, topology.dim - 1, adj, local_values.astype(np.int32)
            )
            ft.name = "Facet tags"
        else:
            ft = meshtags(
