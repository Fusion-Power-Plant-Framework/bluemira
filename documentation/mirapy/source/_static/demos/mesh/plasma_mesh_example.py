...
#Plasma parameters
data['kappaXU']     = 1.68;             # (-)   Upper plasma elongation
data['kappaXL']     = 1.88;             # (-)   Lower plasma elongation
data['deltaXU']     = 0.50;             # (-)   Upper plasma triangularity
data['deltaXL']     = 0.50;             # (-)   Lower plasma triangularity
...
# visualization.
m = meshing.Mesh(meshfile=["Mesh1D.geo_unrolled", "Mesh1D.msh"],
                 mesh_dim=1)
...
m.embed = [(Base.Vector(9., 0., 0.), 0.01),
          (Base.Vector(8., -4, 0), 0.9)]

# exporting test with different format
m.meshfile = ["Mesh2D.geo_unrolled", "Mesh2D.msh",
              "Mesh2D.step", "Mesh2D.brep"]
...
boundaries_mvc = dolfin.MeshValueCollection("size_t", mesh2d, dim=mesh2d.topology().dim()-1)
