...
@app.cell
def _(BluemiraWire, GeometryParameterisation, distance_to):
    ...
    return distance_constraint, f_objective
...
@app.cell
def _(PlotOptions, PrincetonD, boundary, plasma, plot_2d, result):
    geom = PrincetonD()
...
def _(
    ...
    wall_boundary_1 = PrincetonD({
        "x1": {"value": 4, "upper_bound": 6},
        "x2": {"value": 12, "lower_bound": 10},
    })
    opt_problem = ContractLengthGOP(plasma, min_distance)
    result_1 = opt_problem.optimise(
        wall_boundary_1, algorithm="SLSQP", opt_conditions={"ftol_abs": 1e-06}
    )
    print("Optimised parameterisation:")
    print(result_1.geom.variables)
    boundary_1 = result_1.geom.create_shape()
    print("Length of wall    :", boundary_1.length)
    print("Distance to plasma:", distance_to(boundary_1, plasma)[0])
