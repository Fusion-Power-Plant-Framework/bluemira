...
filename = (
    ...
    + "/example_stellarator_nurbs.json"
)
# Read in the json data
json_data = read_json(filename)
# Create a plasma surface from NURBS surface data
plasma_surface = make_bsplinesurface(
    poles=json_data["poles2d"],
    mults_u=json_data["mults_u"],
    mults_v=json_data["mults_v"],
    knot_vector_u=json_data["internal_knot_vector_u"],
    knot_vector_v=json_data["internal_knot_vector_v"],
    degree_u=json_data["degree_u"],
    degree_v=json_data["degree_v"],
    weights=json_data["weights_reshaped"],
    periodic=False,
    check_rational=False,
)

shell = BluemiraShell(plasma_surface)
solid = BluemiraSolid(shell)
# Show the CAD
show_cad([solid])
