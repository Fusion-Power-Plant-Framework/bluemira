...
tag_for_material = ([10, 11, 12] + list(range(30, 45)) + list(range(50, 65)))
##tag_for_pressure = 8
tag_for_body_load = list(range(50, 65))
slip_tags = [1, 2]
...
solver.set_materials({tag: (200e9, 0.3) for tag in tag_for_material})

# --- Apply pressure on boundary tag 8 (e.g., self-weight or traction)
##solver.set_normal_load(1e6, tag=tag_for_pressure)

# --- Apply body load on cable tags (e.g., self-weight or traction)
solver.set_body_load(fx=0.0, fy=10000.0, tags= tag_for_body_load)
...
solver.export_vtk("WP-sec_PS.pvd", uh, strain, stress)
