...
def test_biot_savart_loop():
    ...
    loop = Loop(xc, yc, 0)
    gfl = BiotSavartLoop(loop, radius)

    Bx2, Bz2 = np.zeros((nx, nz)), np.zeros((nx, nz))
    for i in range(nx):
        for j in range(nz):
            bx, _, bz = gfl.field([grid.x_1d[i], 0, grid.z_1d[j]])
            Bx2[i, j] = bx
            Bz2[i, j] = bz
    ...
    for i in range(100):
        bz_differential[i] = current * gfl.field(centreline[i])[2]
...
def test_green_field_loop():
    ...
    # Differential Biot-Savart approach
    xc, yc = circle_seg(x_coil, h=[0, z_coil], npoints=1000)
    loop = Loop(x=xc, y=yc)
    bsl = BiotSavartLoop(loop, radius)

    Bx2, Bz2 = np.zeros((nx, nz)), np.zeros((nx, nz))
    for i in range(nx):
        for j in range(nz):
            bx, _, bz = bsl.field([grid.x_1d[i], 0, grid.z_1d[j]])
            Bx2[i, j] = bx
            Bz2[i, j] = bz
    ...
    # Note that we're using a lot of points on our circle...
    # But the errors are predominantly around the centre of the coil
    # So a for 1000 point circle we can get less than 0.2 % discrepancy with
    # an analytical solution (Green's functions for Bx and Bz)
    assert np.amax(err_Bx) <= 0.013
    assert np.amax(err_Bz) <= 0.61
    assert np.amax(err_Bp) <= 0.0033
    assert rms_Bx <= 5e-7
    assert rms_Bz <= 6e-7
    assert rms_Bp <= 5e-7
    assert np.allclose(Bp, Bp2, rtol=5e-5)
    ...
    bz_differential = np.zeros(100)
    for i in range(100):
        bz_differential[i] = current * bsl.field(centreline[i])[2]
    ...
    assert np.allclose(bz_analytical, bz_differential, rtol=5e-4)
