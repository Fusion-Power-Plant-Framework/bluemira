...
class TestFindLCFSSeparatrix:
    def test_other_grid(self):
        sof = Equilibrium.from_eqdsk(Path(DATA, "eqref_OOB.json"), from_cocos=3)
        psi = sof.psi()
    ...
    def test_double_null(self):
        sof = Equilibrium.from_eqdsk(Path(DATA, "DN-DEMO_eqref.json"), from_cocos=3)
        psi = sof.psi()
        o_points, x_points = sof.get_OX_points(psi)
        grid_tol = np.hypot(sof.grid.dx, sof.grid.dz)
        for tolerance in [1e-6, 1e-7, 1e-8, 1e-9]:
...
class TestGetLegs:
    @classmethod
    def setup_class(cls):
        cls.sn_eq = Equilibrium.from_eqdsk(Path(DATA, "eqref_OOB.json"), from_cocos=3)
        cls.dn_eq = Equilibrium.from_eqdsk(
            Path(DATA, "DN-DEMO_eqref.json"), from_cocos=3
        )
